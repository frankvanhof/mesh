...
public class HandlerUtilities {
	...
	public <RM extends RestModel> void lockingAsyncTx(InternalActionContext ac, TxAction<RM> handler, Consumer<RM> action) {
		async(ac, () -> {
			lockClusterWrites();
			try {
				return database.tx(handler);
	...
	public <RM extends RestModel> void lockingAsyncTx(InternalActionContext ac, TxAction1<RM> handler, Consumer<RM> action) {
		async(ac, () -> {
			lockClusterWrites();
			try {
				return database.tx(handler);
